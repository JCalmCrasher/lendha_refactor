...
import { getReconciliation, searchReconciliation } from '../../../../store/reconciliation/reconciliationSlice';
...
function AdminReconciliationPage() {
    ...
    const [,] = useDocumentTitle('Lendha | Admin - Reconciliation');

    const { branches: branchResponse } = useSelector((state) => state.branchSlice);

    const { reconciliations, filteredReconciliation } = useSelector((state) => state.reconciliationSlice);
    ...
    const filterForm = () => {
        ...
        dispatch(searchReconciliation(payload));
    };
    ...
    const fetchReconciliation = async () => {
        dispatch(getReconciliation({ page: currentPage }));
    };
    ...
    useEffect(() => {
        fetchReconciliation();
        dispatch(searchReconciliation({}));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentPage]);

    const totalReconciliation = filteredReconciliation?.total_reconciliations || 0;
    const prevMonthReconciliation = filteredReconciliation?.previous_month_total_reconciliations || 0;
    const reconciliationDifference = filteredReconciliation?.reconciliation_difference || 0;

    const totalRepayment = filteredReconciliation?.total_repayments || 0;
    const prevMonthRepayment = filteredReconciliation?.previous_month_total_repayments || 0;
    const repaymentsDifference = filteredReconciliation?.repayments_difference || 0;

    const performance = filteredReconciliation?.performance || 0;
    const prevMonthPerformance = filteredReconciliation?.previous_month_performance || 0;
    const performanceDifference = filteredReconciliation?.performance_difference || 0;

    return (
        <>
            {isFetching && <Spinner />}

            <div className="admin_reconciliation_page">
                <div className="reconciliation_div">
                    <SubSectionHeader headText="Reconciliation" rule />
                    <div className="mt-4 mb-4">
                        <div className="wrapper">
                            <ReconciliationStat
                                ...
                                value={formatAmount(totalReconciliation)}
                                percentage={`${reconciliationDifference}%`}
                                period={`Last month: ₦${prevMonthReconciliation}`}
                            />
                            <ReconciliationStat
                                icon={
                                    <svg
                                ...
                                period={`Last month: ${formatAmount(prevMonthRepayment)}`}
                            />
                            <ReconciliationStat
                                icon={
                                    <svg
                    ...
                    <Table
                        ...
                        noTableData={(reconciliations?.data || []).length === 0}
                        pageCount={reconciliations?.meta?.last_page}
                        changeData={changeData}
                    >
                        {(reconciliations?.data || []).map((reconciliation, i) => (
                            <tr key={i}>
                                <td>{reconciliation?.name || 'N/A'}</td>
                                <td>{reconciliation?.loan_name || 'N/A'}</td>
                                <td>{reconciliation?.loan_start_date || 'N/A'}</td>
                                <td>{reconciliation?.loan_end_date || 'N/A'}</td>
                                <td>{reconciliation?.principal || 'N/A'}</td>
                                <td>{reconciliation?.outstanding_principal || 'N/A'}</td>
                                <td>{reconciliation?.late_interest || 'N/A'}</td>
                                <td>{reconciliation?.loan_balance || 'N/A'}</td>
                                <td>{reconciliation?.late_days || 'N/A'}</td>
                                <td>{reconciliation?.officer_name || 'N/A'}</td>
                                <td>{reconciliation?.subadmin_name || 'N/A'}</td>
                            </tr>
                        ))}
                    </Table>
                </div>
...
function ReconciliationStat({ icon, title, value, percentage, period }) {
    return (
        <>
